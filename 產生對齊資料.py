...
class 產生對齊資料:
	...
	def 產生(self,來源語料,目標語料,工作目錄=None):
		...
		目標平行語料統計=self.平行語料統計.format(來源語料,目標語料)
		來源對齊詞資料=self.對齊資料.format(來源語料,目標語料)
		目標對齊詞資料=self.對齊資料.format(來源語料,目標語料)
		產生對齊詞資料='{0} {1} {2} {3} > {4}'
		os.system(產生對齊詞資料.format(self.snt2cooc,
			來源詞數統計,目標詞數統計,
			來源平行語料統計,來源對齊詞資料))
		os.system(產生對齊詞資料.format(self.snt2cooc,
			目標詞數統計,來源詞數統計,
		...
		目標對齊結果=self.對齊結果.format(來源語料,目標語料)
		產生對齊句='{0} -S {1} –T {2} –C {3} -CoocurrenceFile {4}  -O {5}'
		os.system(產生對齊句.format(self.gizapp,
			來源詞數統計,目標詞數統計,
			來源平行語料統計,來源對齊詞資料,來源對齊結果))
		os.system(產生對齊句.format(self.gizapp,
			目標詞數統計,來源詞數統計,
			目標平行語料統計,目標對齊詞資料,目標對齊結果))
